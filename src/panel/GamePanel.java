...
import tankwar.Explode;
import tankwar.QuickTank;
import tankwar.Bullet;
import tankwar.DisplayableImage;
import tankwar.Level;
import tankwar.Map;
import tankwar.PlayerTank;
import tankwar.SlowTank;
import tankwar.AbstractTank;
import tankwar.Prop;
import wall.BaseWall;
import wall.Wall;
...
public class GamePanel extends JPanel implements KeyListener {
	...
	private PlayerTank play1, play2;// 玩家1、玩家2
	...
	private volatile List<AbstractTank> allTanks;// 所有坦克集合
	private List<AbstractTank> botTanks;// 电脑坦克集合
	public static int botCount = 20;// 电脑坦克总数
	...
	private List<AbstractTank> playerTanks;// 玩家坦克集合
	private volatile boolean finish = false;// 游戏是否结束
	private BaseWall base;// 基地
	private List<Wall> walls;// 所有墙块
	private List<Explode> boomImage;// 坦克阵亡后的爆炸效果集合
	private Random r = new Random();// 随机数对象
	private int createBotTimer = 0;// 创建电脑坦克计时器
	private PlayerTank survivor;// （玩家）幸存者,用于绘制最后一个爆炸效果
	private List<AudioClip> audios=AudioPath.getAudios();// 所有背景音效的集合
	private Prop tool=Prop.getToolInstance(r.nextInt(500), r.nextInt(500));
...
//		}
		...
		t.start();// 启动线程
		new AudioPlayer(AudioPath.START).new AudioThread().start();// 播放背景音效
		addListener();// 开启监听
	}
	/**
	 * 组件初始化
	...
	private void init() {
		...
		play1 = new PlayerTank(278, 537, ImagePath.PLAYER1_UP_IMAGE_URL, this, TankType.PLAYER1);// 实例化玩家1
		if (gameType == GameType.TWO_PLAYER) {// 如果是双人模式
			play2 = new PlayerTank(448, 537, ImagePath.PLAYER2_UP_IMAGE_URL, this, TankType.PLAYER2);// 实例化玩家2
			playerTanks.add(play2);// 玩家坦克集合添加玩家2
		}
		playerTanks.add(play1);// 玩家坦克集合添加玩家1

		botTanks = new ArrayList<>();// 实例化电脑坦克集合
		botTanks.add(new QuickTank(botX[0], 1, ImagePath.COMMON_UP_IMAGE_URL, this, TankType.COMMONTANK,3));// 在第一个位置添加电脑坦克
		botTanks.add(new QuickTank(botX[1], 1, ImagePath.COMMON_UP_IMAGE_URL, this, TankType.COMMONTANK,3));// 在第二个位置添加电脑坦克
		botTanks.add(new QuickTank(botX[2], 1, ImagePath.COMMON_UP_IMAGE_URL, this, TankType.COMMONTANK,3));// 在第三个位置添加电脑坦克
	...
	private void paintImage() {
		...
		if (botSurplusCount == 0) {// 如果所有电脑都被消灭
			...
			g.setColor(Color.WHITE);// 使用绿色
			g.drawString("胜利 !", 250, 400);// 在指定坐标绘制文字
		...
		if (gameType == GameType.ONE_PLAYER) {// 如果是单人模式
			if (!play1.isAlive()&&play1.getLife()==0) {// 如果玩家1阵亡,并且玩家1的生命数等于0
				stopThread();// 结束游戏帧刷新线程
				boomImage.add(new Explode(play1.x, play1.y));// 添加玩家1爆炸效果
				panitBoom();// 绘制爆炸效果
			...
			if (play1.isAlive() && !play2.isAlive() && play2.getLife()==0) {// 如果玩家1是 幸存者
				...
				stopThread();// 结束游戏帧刷新线程
				boomImage.add(new Explode(survivor.x, survivor.y));// 添加幸存者爆炸效果
				panitBoom();// 绘制爆炸效果
				paintGameOver();// 在屏幕中央绘制game over
				gotoPrevisousLevel();// 重新进入本关卡
			}
		...
		if (!base.isAlive()) {// 如果基地被击中
			stopThread();// 结束游戏帧刷新线程
			paintGameOver();// 在屏幕中央绘制game over
			base.setImage(ImagePath.BREAK_BASE_IMAGE_URL);// 基地使用阵亡图片
			gotoPrevisousLevel();// 重新进入本关卡
		}
	...
	private void paintGameOver() {
		g.setFont(new Font("黑体", Font.BOLD, 60));// 设置绘图字体
		g.setColor(Color.RED);// 设置绘图颜色
		g.drawString("Game Over !", 250, 400);// 在指定坐标绘制文字
		g.drawString("当前关卡："+level, 250, 500);// 在指定坐标绘制文字
		new AudioPlayer(AudioPath.GAMEOVER).new AudioThread().start();//新建一个音效线程，用于播放音效
	}
	...
	private void panitBoom() {
		for (int i = 0; i < boomImage.size(); i++) {// 循环遍历爆炸效果集合
			Explode boom = boomImage.get(i);// 获取爆炸对象
	...
	private void panitBotTanks() {
		for (int i = 0; i < botTanks.size(); i++) {// 循环遍历电脑坦克集合
			QuickTank t = (QuickTank) botTanks.get(i);// 获取电脑坦克对象
			if (t.isAlive()) {// 如果坦克存活
				...
				i--;// 循环变量-1，保证下次循环i的值不会变成i+1，以便有效遍历集合，且防止下标越界
				boomImage.add(new Explode(t.x, t.y));// 在坦克位置创建爆炸效果
	...
	private void panitPlayerTanks() {
		for (int i = 0; i < playerTanks.size(); i++) {// 循环遍历玩家坦克
			PlayerTank t = (PlayerTank) playerTanks.get(i);// 获取玩家坦克对象
			if (t.isAlive()) {// 如果坦克存活
				t.hitTool();//判断是否碰撞到道具
				t.addStar();
				g.drawImage(((DisplayableImage) t).getImage(), t.x,t.y, this);// 绘制坦克
			} else {// 如果坦克阵亡
	...
	private void createBotTank() {
		...
		int flag=r.nextInt(3)+1;
		createBotTimer += FRESHTIME;// 计时器按照刷新时间递增
		// “当场上电脑小于场上最大数时” 并且 “准备上场的坦克数量大于0” 并且 “计时器记录已过去1.5秒钟”
		if (botTanks.size() < botMaxInMap && botReadyCount > 0 && createBotTimer >= 5000 && flag == 1 ) {
			
			Rectangle bornRect = new Rectangle(botX[index], 1, 35, 35);// 创建坦克随机出生区域
			for (int i = 0, lengh = allTanks.size(); i < lengh; i++) {// 循环遍历所有坦克集合
				AbstractTank t = allTanks.get(i);// 获取坦克对象
				if (t.isAlive() && ((DisplayableImage) t).hit(bornRect)) {// 如果场上存在与随机位置重合并存活的坦克
					return;// 结束方法
				}
			}
			botTanks.add(new QuickTank(botX[index], 1, ImagePath.BOT_UP_IMAGE_URL, GamePanel.this, TankType.QUICKTANK,5));// 在随机位置创造电脑坦克
			new AudioPlayer(AudioPath.ADD).new AudioThread().start();
			botReadyCount--;// 准备上场电脑数量-1
			createBotTimer = 0;// 产生电脑计时器重新计时
			flag=2;
		}
		else if(botTanks.size() < botMaxInMap && botReadyCount > 0 && createBotTimer >= 5000 && flag == 2 ) {
			Rectangle bornRect = new Rectangle(botX[index], 1, 35, 35);// 创建坦克随机出生区域
			for (int i = 0, lengh = allTanks.size(); i < lengh; i++) {// 循环遍历所有坦克集合
				AbstractTank t = allTanks.get(i);// 获取坦克对象
				if (t.isAlive() && ((DisplayableImage) t).hit(bornRect)) {// 如果场上存在与随机位置重合并存活的坦克
			...
			botTanks.add(new QuickTank(botX[index], 1, ImagePath.SLOW_UP_IMAGE_URL, GamePanel.this, TankType.SLOWTANK,1));// 在随机位置创造电脑坦克
			new AudioPlayer(AudioPath.ADD).new AudioThread().start();
			botReadyCount--;// 准备上场电脑数量-1
			createBotTimer = 0;// 产生电脑计时器重新计时
		}
		else if(botTanks.size() < botMaxInMap && botReadyCount > 0 && createBotTimer >= 5000 && flag == 3 ) {
			Rectangle bornRect = new Rectangle(botX[index], 1, 35, 35);// 创建坦克随机出生区域
			for (int i = 0, lengh = allTanks.size(); i < lengh; i++) {// 循环遍历所有坦克集合
				AbstractTank t = allTanks.get(i);// 获取坦克对象
				if (t.isAlive() && ((DisplayableImage) t).hit(bornRect)) {// 如果场上存在与随机位置重合并存活的坦克
			...
			botTanks.add(new QuickTank(botX[index], 1, ImagePath.COMMON_UP_IMAGE_URL, GamePanel.this, TankType.COMMONTANK,3));// 在随机位置创造电脑坦克
			new AudioPlayer(AudioPath.ADD).new AudioThread().start();
			botReadyCount--;// 准备上场电脑数量-1
			createBotTimer = 0;// 产生电脑计时器重新计时
		}
...
//		Thread jump = new JumpPageThead(Level.level.previsousLevel());
	...
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_SPACE:// 如果按下的是空格
			space_key = true;// 空格按下标志为true
			break;
	...
	public void keyReleased(KeyEvent e) {
		...
		case KeyEvent.VK_NUMPAD0:// 如果抬起的是小键盘1
	 ...
	 * @return 面板中所有存在的电脑坦克
	 */
	public List<AbstractTank> getBotTanks(){
		return botTanks;
	}
	/**
	 * 获取到游戏面板的道具对象
	 ...
	 * @return
	 */
	public List<AbstractTank> getPlayerTanks() {
