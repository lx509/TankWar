...
public class SlowTank extends QuickTank {
	...
	public SlowTank(int x, int y, String url,GamePanel gamePanel, TankType type,int speed) {
		super(x, y, ImagePath.SLOW_DOWN_IMAGE_URL, gamePanel, type,1);
		...
		for (int i = 0, lengh = tanks.size(); i < lengh; i++) {// 遍历坦克集合
			...
			if (!this.equals(t)) {// 如果此坦克对象与本对象不是同一个
				if (t.isAlive() && ((DisplayableImage) t).hit(next)) {// 如果对方说是存活的，并且与本对象发生碰撞
					if (t instanceof SlowTank) {// 如果对方也是电脑
						dir = randomDirection();// 随机调整移动方向
					}
					return true;// 发生碰撞
				}
	...
	public void addBomb() {
		...
		for(int i=0;i<botTanks.size();i++) {
			SlowTank b=(SlowTank) botTanks.get(i);
			b.setAlive(false);
		}
	}
