...
import wall.BrickWall;
import wall.GrassWall;
import wall.IronWall;
import wall.RiverWall;
import wall.Wall;
		...
		for (int i = 0, lengh = tanks.size(); i < lengh; i++) {// 遍历坦克集合
			...
			if (t.isAlive() && this.hit((DisplayableImage) t)) {// 如果坦克是存活的并且子弹击中了坦克
				...
				case PLAYER2:// 如果是玩家2的子弹
					...
					} else if (t instanceof PlayerTank) {// 如果击中的是玩家
						//alive = false;// 子弹销毁
						this.dispose();
					}
					break;
				case QUICKTANK:// 如果是电脑的子弹
					if (t instanceof QuickTank) {// 如果击中的坦克是电脑
						//alive = false;// 子弹销毁
						this.dispose();
					} else if (t instanceof PlayerTank) {// 如果击中的是玩家
		...
		for (int i = 0, lengh = walls.size(); i < lengh; i++) {// 遍历所有墙块
			...
			if (this.hit(w)) {// 如果子弹击中墙块
				if (w instanceof BrickWall) {// 如果是砖墙
					new AudioPlayer(AudioPath.HIT).new AudioThread().start();
					this.dispose();
					//alive = false;// 子弹销毁
