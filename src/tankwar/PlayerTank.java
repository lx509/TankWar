 ...
 *
 */
public class PlayerTank extends DisplayableImage implements AbstractTank{
	...
	public PlayerTank(int x, int y, String url, GamePanel gamePanel, TankType type) {
		super(x, y, url);
	 ...
	 */
	 public boolean hitTank(int x, int y) {
		...
		List<AbstractTank> tanks = gamePanel.getTanks();// 获取所有坦克
		for (int i = 0, lengh = tanks.size(); i < lengh; i++) {// 遍历所有坦克
			AbstractTank t = tanks.get(i);// 获取tank对象
			if (!this.equals(t)) {// 如果此坦克与自身不是同一个对象
				if (t.isAlive() && ((DisplayableImage) t).hit(next)) {// 如果此坦克存活并且与自身相撞
					return true;// 返回相撞
				}
			}
		}
	...
	public final boolean hitTool() {
		Prop tool = gamePanel.getTool();
		List<AbstractTank> tanks = gamePanel.getPlayerTanks();// 获取所有玩家坦克
		//System.out.println("hitTool()方法执行");
		for (int i = 0, lengh = tanks.size(); i < lengh; i++) {// 遍历所有玩家坦克
			PlayerTank t = (PlayerTank) tanks.get(i);// 获取tank对象
			//System.out.println(tool.getRect().toString());
			if (t.type==TankType.PLAYER1||t.type==TankType.PLAYER2) {// 如果此坦克是玩家坦克1或者是玩家坦克2
				if (t.isAlive() && ((DisplayableImage) t).hit(tool)&&tool.getAlive()) {// 如果此坦克存活并且与道具相撞
